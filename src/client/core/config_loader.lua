...
PowerACConfig = nil
PowerACLoaded = false
PowerACProtectionSettings = {}
PowerACInitCalled = false
PowerACAdminList = {}
PowerACLastAdminUpdate = 0

function ConfigLoader.initialize()
    if PowerACInitCalled then return end
    PowerACInitCalled = true
    ...
    while not PowerACLoaded and attempts < maxAttempts do
        Wait(1000)
        attempts = attempts + 1
        if not PowerACLoaded then
            TriggerServerEvent("requestConfig")
        end
    end
end
...
RegisterNetEvent("receiveConfig", function(serverConfig)
    ...
    PowerACConfig = serverConfig
    PowerAC = serverConfig
...
function ConfigLoader.get(key, default)
    if not PowerACLoaded or not PowerACConfig then
        return default
    end
...
function ConfigLoader.is_loaded()
    return PowerACLoaded
end

---@description Get the entire config table
---@return table config The config table
function ConfigLoader.get_config()
    return PowerACConfig
end

---@description Get the PowerAC configuration
---@return table powerac The PowerAC configuration
function ConfigLoader.get_powerac()
    return PowerAC
end
...
local function build_blacklist_hashes()
    ...
    if not PowerAC or not PowerAC.Protection then
        return
    end
    
    add_blacklist_hashes(PowerAC.Protection.BlacklistedObjects, blacklist_model_hashes.objects)
    add_blacklist_hashes(PowerAC.Protection.BlacklistedVehicles, blacklist_model_hashes.vehicles)
    add_blacklist_hashes(PowerAC.Protection.BlacklistedPeds, blacklist_model_hashes.peds)
end

---@description Get protection setting directly from PowerAC.Protection.Simple
...
local function get_from_simple_protection(name, property)
    if not PowerAC or not PowerAC.Protection or not PowerAC.Protection.Simple then
        return nil
    end
    
    for _, v in pairs(PowerAC.Protection.Simple) do
        if v.protection == name then
            if property == "time" and type(v.time) ~= "number" and PowerAC.BanTimes then
                return PowerAC.BanTimes[v.time]
            elseif property == "webhook" and v.webhook == "" and PowerAC.Webhooks then
                return PowerAC.Webhooks.Simple
            else
                return v[property]
            end
        end
...
function ConfigLoader.get_protection_setting(name, property)
    ...
    if PowerACProtectionSettings[name] and PowerACProtectionSettings[name][property] ~= nil then
        return PowerACProtectionSettings[name][property]
    end
    
    if PowerACLoaded and PowerAC and PowerAC.Protection and PowerAC.Protection.Simple then
        for _, v in pairs(PowerAC.Protection.Simple) do
            if v.protection == name then
                local time = v.time
                if type(time) ~= "number" and PowerAC.BanTimes then
                    time = PowerAC.BanTimes[v.time]
                end
                
                local webhook = v.webhook
                if webhook == "" and PowerAC.Webhooks then
                    webhook = PowerAC.Webhooks.Simple
                end
                ...
                PowerACProtectionSettings[name] = settings
...
function ConfigLoader.process_config(config)
    ...
    PowerAC = config 
    local PowerAC = PowerAC
    
    PowerACProtectionSettings = {}
    
    if PowerAC and PowerAC.Protection and type(PowerAC.Protection.Simple) == "table" then
        for k, v in pairs(PowerAC.Protection.Simple) do
            if PowerAC.Webhooks and v.webhook == "" then
                PowerAC.Protection.Simple[k].webhook = PowerAC.Webhooks.Simple
            end
            if PowerAC.BanTimes and type(v.time) ~= "number" then
                PowerAC.Protection.Simple[k].time = PowerAC.BanTimes[v.time]
            end
            
            local name = PowerAC.Protection.Simple[k].protection
            local dispatch = PowerAC.Protection.Simple[k].dispatch
            local default = PowerAC.Protection.Simple[k].default
            local defaultr = PowerAC.Protection.Simple[k].defaultr
            local tolerance = PowerAC.Protection.Simple[k].tolerance
            local defaults = PowerAC.Protection.Simple[k].defaults
            local time = PowerAC.Protection.Simple[k].time
            if PowerAC.BanTimes and type(time) ~= "number" then
                time = PowerAC.BanTimes[v.time]
            end
            local limit = PowerAC.Protection.Simple[k].limit or 999
            local webhook = PowerAC.Protection.Simple[k].webhook
            if PowerAC.Webhooks and webhook == "" then
                webhook = PowerAC.Webhooks.Simple
            end
            local enabled = PowerAC.Protection.Simple[k].enabled
...
function ConfigLoader.process_blacklist_category(category)
    local PowerAC = PowerAC
    if not PowerAC or not PowerAC.Protection or type(PowerAC.Protection[category]) ~= "table" then
        return
    end
    
    for k, v in pairs(PowerAC.Protection[category]) do
        if PowerAC.Webhooks and v.webhook == "" then
            PowerAC.Protection[category][k].webhook = PowerAC.Webhooks[category]
        end
        if PowerAC.BanTimes and type(v.time) ~= "number" then
            PowerAC.Protection[category][k].time = PowerAC.BanTimes[v.time]
        end
...
function ConfigLoader.is_whitelisted(player_id)
    ...
    if currentTime - PowerACLastAdminUpdate > 60000 then
        TriggerServerEvent("PowerAC:RequestAdminList")
        PowerACLastAdminUpdate = currentTime
    end
    
    if PowerACAdminList[tostring(player_id)] then
        return true
    end
...
RegisterNetEvent("PowerAC:ReceiveAdminList", function(adminList)
    PowerACAdminList = adminList
    PowerACLastAdminUpdate = GetGameTimer()
end)

RegisterNetEvent('PowerAC:ReturnMenuAdminStatus', function(request_id, result)
...
function ConfigLoader.is_menu_admin(player)
    ...
    TriggerServerEvent('PowerAC:RequestMenuAdminStatus', player, request_id)
...
RegisterClientCallback({
    eventName = 'PowerAC:RequestScreenshotUpload',
...
RegisterClientCallback({
    eventName = 'PowerAC:CaptureClientScreenshot',
