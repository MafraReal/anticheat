...
function Cache.UpdateAll()
    ...
    if _G.PowerACAdminList then
        Cache.Values.isAdmin = _G.PowerACAdminList[tostring(GetPlayerServerId(PlayerId()))] == true
    end
...
function Cache.RequestPermissionCheck()
    TriggerServerEvent("PowerAC:RequestPermissions")
    local currentTime = GetGameTimer()
...
RegisterNetEvent("PowerAC:ReceivePermissions", function(permissions)
