...
local function normalizePedProofs(ped)
    -- Fix: GetEntityProofs returns booleans (not 1), so the old == 1 test never matched; we only check bulletProof because PowerAC intentionally sets explosionProof every 5s (false positive otherwise).
