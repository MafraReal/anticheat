...
local function can_punish_now(playerId, reason)
    local min_seconds = tonumber(PowerAC and PowerAC.MinimumOnlineSecondsBeforeBan) or 0
    if min_seconds <= 0 then return true end

    local joined_at = Heartbeat.playerJoinTime[playerId]
        or (_G.PowerAC_PlayerJoinedAt and _G.PowerAC_PlayerJoinedAt[playerId])
...
local function load_config()
    local config = (PowerAC and PowerAC.Module and PowerAC.Module.Heartbeat) or {}
            ...
            for _, pid in ipairs(challenged) do
                if is_ready_for_alive_check(pid, checked_at) then
                    if not Heartbeat.alive[pid] then
                        ...
                        -- Fix: we no longer rely on heartbeat freshness (spoofable); repeated nonce-challenge failures are the reliable proof that no PowerAC client is running.
