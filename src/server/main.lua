...
local function setupErrorHandler()
    PowerACErrorHandler = function(err)
    ...
    AddEventHandler('onServerResourceStart', function(resource)
        if resource == GetCurrentResourceName() then
            ...
            error = function(err, level)
                if PowerACErrorHandler then
                    PowerACErrorHandler(err)
                end
...
local function registerServerCommands()
    ...
    RegisterCommand("securebanlist", function(source, args, rawCommand)
        ...
        print("===== PowerAC Ban List (" .. #bans .. " most recent) =====")
    ...
    RegisterCommand("securehelp", function(source, args, rawCommand)
        ...
        print("===== PowerAC Anti-Cheat Commands =====")
    ...
    RegisterCommand("securestats", function(source, args, rawCommand)
        ...
        print("===== PowerAC System Statistics =====")
...
local function main()
  ...
  ^7]])
    ...
    print("^8╔══════════════════════════════════════════════════════════════════════════╗^7")
    print("^8║                  ^2PowerAC AntiCheat V1.5.0 Initializing^8               ║^7")
    print("^8╚══════════════════════════════════════════════════════════════════════════╝^7")
    ...
    AddEventHandler("onResourceStop", function(resource_name)
        if resource_name == GetCurrentResourceName() then
            logger.info("PowerAC AntiCheat is stopping...")
            logger.warn("PowerAC AntiCheat is stopping...")
        end
    end)

    local player_joined_at = {}
    _G.PowerAC_PlayerJoinedAt = player_joined_at
    ...
    RegisterNetEvent("PowerAC:ClientLog", function(level, message)
        local source = source
        local player_name = GetPlayerName(source) or "Unknown"
    ...
    initialized = true
    print("\n^8╔══════════════════════════════════════════════════════════════════════════╗^7")
    print("^8║              ^2PowerAC AntiCheat V1.5.0 Loaded Successfully^8            ║^7")
    ...
    DiscordLogger.log_system(
        "AntiCheat Started",
        "PowerAC AntiCheat V1.5.0 has been successfully initialized.",
        {
    ...
    logger.info("PowerAC AntiCheat V1.5.0 initialized successfully")
end

local function PrintBanner()
    print("^4==============================================================")
    print("^4   ____                               _     ____  ")
    print("^4  |  _ \\  ___  _ __  _ __ ___   _ __  / \\   / ___| ")
    print("^4  | |_) |/ _ \\| '_ \\| '__/ _ \\ | '__|/ _ \\ | |     ")
    print("^4  |  __/| (_) | |_) | | | (_) || |  / ___ \\| |___  ")
    print("^4  |_|    \\___/| .__/|_|  \\___/ |_| /_/   \\_\\\\____| ")
    print("^4              |_|                                  ")
    print("^4             SECURE ANTICHEAT LICENSE SYSTEM V2.0     ")
    print("^4==============================================================^7")
end

local function ShutdownServer(reason)
    print("^1[PowerAC - BLOCKED] =============================================^7")
    print("^1[PowerAC - BLOCKED] DETTAGLI: " .. reason .. "^7")
    print("^1[PowerAC - BLOCKED] Impossibile avviare l'anticheat senza licenza.^7")
    print("^1[PowerAC - BLOCKED] Spegnimento risorsa in corso...^7")
    print("^1[PowerAC - BLOCKED] =============================================^7")
    
    -- Ferma la risorsa FiveM in modo che non carichi l'anticheat vero e proprio
...
CreateThread(function()
    Wait(2000)
    PrintBanner()
    print("^3[PowerAC] Connessione al server di autenticazione in corso...^7")

    local LicenseKey = (Config and Config.LicenseKey) or "VAC-INSERISCI-QUI" 
    local PanelURL = (Config and Config.PanelURL) or "http://localhost:3000" 

    local payload = json.encode({
        license = LicenseKey
    })

    PerformHttpRequest(PanelURL .. "/api/validate", function(statusCode, responseText, headers)
        if statusCode == 200 then
            local data = json.decode(responseText)
            if data and data.success then
                print("^2[PowerAC - SUCCESS] Licenza verificata ed attiva! Avvio autorizzato.^7")
                print("^2[PowerAC - INFO] Grado Licenza: ^5" .. tostring(data.license.tier) .. "^7")
                print("^2[PowerAC - INFO] Data Scadenza: ^5" .. tostring(data.license.expiresAt) .. "^7")
                
                -- Sincronizza i parametri anticheat modificati dalla Dashboard Web in tempo reale!
                if data.config then
                    print("^2[PowerAC - CONFIG] Configurazione caricata dal Pannello Web in tempo reale!^7")
                    print("^2[PowerAC - CONFIG] Anti-Noclip: " .. tostring(data.config.antiNoclip))
                    print("^2[PowerAC - CONFIG] Anti-GiveWeapon: " .. tostring(data.config.antiGiveWeapon))
                    print("^2[PowerAC - CONFIG] Anti-Spectate: " .. tostring(data.config.antiSpectate))
                    print("^2[PowerAC - CONFIG] Sensibilita: " .. tostring(data.config.detectionSensitivity) .. "/10")

                    -- Applica alla configurazione locale di PowerAC
                    if Config and Config.Protections then
                        Config.Protections["Anti Noclip"] = data.config.antiNoclip
                        Config.Protections["Anti Spectate"] = data.config.antiSpectate
                        Config.Protections["Anti Freecam"] = data.config.antiSpectate
                        Config.Protections["Anti Give Weapon"] = data.config.antiGiveWeapon
                        Config.Protections["Anti Weapon Pickup"] = data.config.antiGiveWeapon
                    end

                    if Config and Config.Webhooks then
                        if data.config.discordLogWebhook and data.config.discordLogWebhook ~= "" then
                            Config.Webhooks.Detection = data.config.discordLogWebhook
                            Config.Webhooks.Ban = data.config.discordLogWebhook
                            Config.Webhooks.Kick = data.config.discordLogWebhook
                        end
                    end
                end
                
                -- Se la licenza è valida, avvia i moduli anticheat
                main()
            else
                ShutdownServer("Autenticazione Fallita: " .. (data and data.message or "Errore sconosciuto"))
            end
        elseif statusCode == 403 then
            local data = json.decode(responseText)
            ShutdownServer("Errore 403 (IP LOCK): " .. (data and data.message or "Il tuo IP server non corrisponde a quello registrato."))
        elseif statusCode == 402 then
            local data = json.decode(responseText)
            ShutdownServer("Errore 402 (SCADUTA): " .. (data and data.message or "La tua licenza è scaduta. Contatta il supporto per il rinnovo."))
        elseif statusCode == 404 then
            local data = json.decode(responseText)
            ShutdownServer("Errore 404 (LICENZA INESISTENTE): " .. (data and data.message or "Licenza non registrata nel database."))
        else
            ShutdownServer("Impossibile contattare il server delle licenze (HTTP Status " .. tostring(statusCode) .. "). Controlla se la Web Dashboard è online!")
        end
    end, "POST", payload, { ["Content-Type"] = "application/json" })
end)

exports("get_event_protector_config", function()
    local settings = PowerAC and PowerAC.EventProtector or {}
...
exports("module_punish", function(source, reason, webhook, time)
    if PowerAC and PowerAC.Module and PowerAC.Module.ModuleEnabled == false then
        return true
    end
    ...
    if resource_to_check then
        ...
        if resource_to_check == GetCurrentResourceName() then
            logger.debug("Detection from PowerAC itself, ignoring")
            return true
        end
...
RegisterNetEvent("PowerAC:Server:Methods:PunishPlayer", function(id, reason, webhook, time)
    local source = source
    ...
    local min_seconds = tonumber(PowerAC.MinimumOnlineSecondsBeforeBan) or 0
    if min_seconds > 0 and _G.PowerAC_PlayerJoinedAt then
        local joined_at = _G.PowerAC_PlayerJoinedAt[source]
        if joined_at then
