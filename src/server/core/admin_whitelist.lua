...
function AdminWhitelist.getManualAdmin(source)
    if not _G.PowerAC then return false end

    local identifiers = get_identifiers(source)
    local licenses    = (_G.PowerAC.AdminMenu and _G.PowerAC.AdminMenu.Licenses) or {}
    local manualAdmins = _G.PowerAC.Admins or {}
...
function AdminWhitelist.isWhitelisted(source)
    ...
    if not result and _G.PowerAC and _G.PowerAC.Whitelisted then
        local src = tonumber(source)
        for _, id in ipairs(_G.PowerAC.Whitelisted) do
...
function AdminWhitelist.checkAndAddAdmin(source)
    ...
    if AdminWhitelist.isAdmin(source) then
        if _G.PowerAC and _G.PowerAC.Whitelisted then
            local src = tonumber(source)
            for _, id in ipairs(_G.PowerAC.Whitelisted) do
                if tonumber(id) == src then return end
            end
            table.insert(_G.PowerAC.Whitelisted, src)
...
function AdminWhitelist.initialize()
    ...
    if not _G.PowerAC then _G.PowerAC = {} end
    if not _G.PowerAC.Whitelisted then _G.PowerAC.Whitelisted = {} end
    ...
    RegisterNetEvent("PowerAC:CheckWhitelist", function()
        local src = source
        TriggerClientEvent("PowerAC:WhitelistResponse", src, AdminWhitelist.isWhitelisted(src))
    end)

    RegisterNetEvent("PowerAC:RequestAdminList", function()
        local src = source
        local adminList = {}

        if AdminWhitelist.isWhitelisted(src) and _G.PowerAC and _G.PowerAC.Whitelisted then
            for _, adminId in ipairs(_G.PowerAC.Whitelisted) do
        ...
        TriggerClientEvent("PowerAC:ReceiveAdminList", src, adminList)
    end)
    ...
    AddEventHandler("playerDropped", function()
        ...
        if _G.PowerAC and _G.PowerAC.Whitelisted then
            for i = #_G.PowerAC.Whitelisted, 1, -1 do
                if tonumber(_G.PowerAC.Whitelisted[i]) == tonumber(src) then
                    table.remove(_G.PowerAC.Whitelisted, i)
                    break
                end
            end
        end
...
RegisterNetEvent("PowerAC:RequestPermissions", function()
    local src = source
    if not src or src <= 0 or not GetPlayerName(src) then return end
    TriggerClientEvent("PowerAC:ReceivePermissions", src, AdminWhitelist.getPlayerPermissions(src))
end)
