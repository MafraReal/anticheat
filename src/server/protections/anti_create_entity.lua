...
function AntiCreateEntity.initialize()
        if PowerAC and PowerAC.Module and PowerAC.Module.Entity and PowerAC.Module.Entity.SecurityWhitelist then
            for _, entry in ipairs(PowerAC.Module.Entity.SecurityWhitelist) do
                if entry and entry.resource and entry.whitelist then
                    AntiCreateEntity.resourceWhitelist[entry.resource] = entry.whitelist
                end
            end
            logger.info("Loaded " .. tostring(#PowerAC.Module.Entity.SecurityWhitelist) .. " whitelisted resources for entity security")
        end


    RegisterNetEvent("PowerAC:Server:Methods:ModulePunish", function(screenshot, reason, webhook, time)
        local src = source
        ...
        logger.warn(string.format("[PowerAC] Entity Security: Player %s (%s) %s",
        ...
        print("[PowerAC] Incase this is a false ban go to config.lua and in it search PowerAC.Module incase this is an event ban go to Events and then to whitelist and add the event")
        print("[PowerAC] Incase its an entity ban go to Entity and SecurityWhitelist and add the resource name")
        if not screenshot and PowerAC.Module.Entity.TakeScreenshot then
    ...
    AddEventHandler("PowerAC:Server:Methods:Entity:CreateServer", function(entityId, resourceName, modelHash)
    ...
    RegisterNetEvent("PowerAC:Server:Methods:Entity:Create", function(entityId, resourceName, modelHash)
        local src = source
        if not src or src <= 0 then return end
    ...
    AddEventHandler('entityCreated', function(entity)
        ...
        if (population == 7 or population == 0) and owner ~= 0 and owner ~= -1 then
            ...
            elseif owner and modelHash then
                TriggerClientEvent("PowerAC:CheckEntityResource", owner, NetworkGetNetworkIdFromEntity(entity), modelHash)
            end
        end
    end)
