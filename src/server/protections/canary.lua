...
local function load_config()
    local heartbeat = (PowerAC and PowerAC.Module and PowerAC.Module.Heartbeat) or {}
...
local function can_punish_now(src, reason)
    ...
    local joined_at = _G.PowerAC_PlayerJoinedAt and _G.PowerAC_PlayerJoinedAt[src]
...
function Canary.initialize()
    ...
    RegisterNetEvent("keepalive:ssMissing", function()
        ...
        if session.missing_strikes >= 2 then
            punish(src, "PowerAC stopped on client side")
        else
            assign_session(src, session)
            logger.warn(("Canary PowerAC missing report from %s; waiting for confirmation")
                :format(tostring(src)))
        end
    end)
