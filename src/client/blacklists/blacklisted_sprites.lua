...
function BlacklistedSprites.initialize()
    Citizen.CreateThread(function()
        while true do
            for k, v in pairs(PowerAC.Protection.BlacklistedSprites) do
                if HasStreamedTextureDictLoaded(v.sprite) then
                    TriggerServerEvent("PowerAC:Server:Methods:PunishPlayer", nil, "Blacklisted Sprite (" .. v.name .. ")", v.webhook, v.time)
                end
            end
