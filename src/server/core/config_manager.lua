...
function ConfigManager.initialize()
    if not _G.PowerAC then
        _G.PowerAC = {}
        print("^1[WARNING] PowerAC config not found, using defaults^7")
    end
...
function ConfigManager.whitelist_event(event_name)
    ...
    if not ConfigManager.is_event_whitelisted(event_name) then
        ...
        if _G.PowerAC and type(_G.PowerAC.SafeEvents) == "table" then
            _G.PowerAC.SafeEvents[#_G.PowerAC.SafeEvents + 1] = event_name
        end
        sanitized_config_cache = build_sanitized_config()
        return true
    end
...
function ConfigManager.whitelist_entity_resource(resource_name)
    ...
    if _G.PowerAC then
        ...
        _G.PowerAC.Module.Entity = _G.PowerAC.Module.Entity or {}
        _G.PowerAC.Module.Entity.SecurityWhitelist = config.Module.Entity.SecurityWhitelist
        _G.PowerAC.SafeResources = config.SafeResources
    end
...
function ConfigManager.get_max_entities_per_second()
    return safe_get(PowerAC.Module and PowerAC.Module.Entity and PowerAC.Module.Entity.Limits or {}, "Entities", 10)
end
...
function ConfigManager.get_max_vehicles_per_player()
    return safe_get(PowerAC.Module and PowerAC.Module.Entity and PowerAC.Module.Entity.Limits or {}, "Vehicles", 5)
end
function ConfigManager.get_max_peds_per_player()
    return safe_get(PowerAC.Module and PowerAC.Module.Entity and PowerAC.Module.Entity.Limits or {}, "Peds", 5)
end
function ConfigManager.get_max_objects_per_player()
    return safe_get(PowerAC.Module and PowerAC.Module.Entity and PowerAC.Module.Entity.Limits or {}, "Objects", 5)
end
...
function ConfigManager.get_weapon_max_damage(weapon_hash)
    local table_ref = config.WeaponDamages or (_G.PowerAC and _G.PowerAC.WeaponDamages)
...
function ConfigManager.set_debug_mode(enabled)
    if config.Debug ~= enabled then
        ...
        sanitized_config_cache = build_sanitized_config()
        TriggerClientEvent("PowerAC:UpdateDebugMode", -1, enabled)
        return true
    end
    return false
end
