...
function AntiOcr.initialize()
    if not PowerAC.OCR then return end

    local ocrWords = {}
    local screenshotInterval = 5500

    if type(PowerAC.OCR) == "table" then
        if PowerAC.OCR.Words then
            ocrWords = PowerAC.OCR.Words
            screenshotInterval = PowerAC.OCR.ScreenshotInterval or 5500
        else
            screenshotInterval = PowerAC.OCR.ScreenshotInterval or 5500

            for key, value in pairs(PowerAC.OCR) do
        ...
        if data.image and data.text then
            for index, word in next, ocrWords, nil do
                if string.find(string.lower(data.text), string.lower(word)) then
                    ...
                    TriggerServerEvent("PowerAC:Server:Methods:PunishPlayer", nil, "Found word on screen [OCR]: " .. word)
                    break
                end
            end
        end
    ...
    Citizen.CreateThread(function()
        ...
        if not exports['screenshot-basic'] or type(exports['screenshot-basic'].requestScreenshot) ~= "function" then
            if PowerAC and PowerAC.Debug then
                print("^3[PowerAC] Anti OCR disabled: 'screenshot-basic' resource not running.^7")
            end
            return
        end
