...
/**
 * Check if manifest has old PowerAC include
 * @param {string} manifestContent - Content of manifest file
 * @return {boolean} Whether it has the old PowerAC include
 */
function hasOldPowerAC(manifestContent) {
    return manifestContent.includes(`shared_script "@${CURRENT_RESOURCE_NAME}/module.lua"`);
}

/**
 * Check if manifest has new PowerAC includes
 * @param {string} manifestContent - Content of manifest file
 * @return {boolean} Whether it has the new PowerAC includes
 */
function hasNewPowerAC(manifestContent) {
...
function hasKeyFile(manifestContent) {
    return manifestContent.includes(`file "@${CURRENT_RESOURCE_NAME}/powerac.key"`);
}

/**
 * Add PowerAC includes to manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content
 */
function addPowerAC(manifestContent) {
    if (manifestContent.includes(`name "${CURRENT_RESOURCE_NAME}"`) || 
        manifestContent.includes(`name '${CURRENT_RESOURCE_NAME}'`)) {
        return manifestContent;
    }
    
    if (hasNewPowerAC(manifestContent) && hasKeyFile(manifestContent)) {
        return manifestContent;
    }
    
    if (hasOldPowerAC(manifestContent)) {
...
/**
 * Remove PowerAC references from manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content with PowerAC removed
 */
function removePowerAC(manifestContent) {
    manifestContent = manifestContent.replace(`shared_script "@${CURRENT_RESOURCE_NAME}/module.lua"`, '');
    manifestContent = manifestContent.replace(`shared_script "@${CURRENT_RESOURCE_NAME}/src/module/module.lua"`, '');
    manifestContent = manifestContent.replace(``, '');
    manifestContent = manifestContent.replace(`file "@${CURRENT_RESOURCE_NAME}/powerac.key"`, '');
...
/**
 * Update PowerAC references in manifest content
 * @param {string} manifestContent - Content of manifest file
 * @return {string} Updated manifest content
 */
function updatePowerAC(manifestContent) {
    if (hasNewPowerAC(manifestContent) || !hasOldPowerAC(manifestContent)) {
        return manifestContent;
    }
...
function checkAndFixDefaultKey() {
    const keyPath = path.join(RESOURCES_PATH, 'powerac.key');
    ...
    try {
        if (fs.existsSync(keyPath)) {
            ...
            if (isDefaultKey) {
                Logger.critical(`Regenerating powerac.key file due to default key...`);
                ...
                if (verifyKey === newKey) {
                    ...
                    Logger.info(`New key will be used by all PowerAC components.`);
            ...
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new powerac.key file with secure random key.`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0); 
            }, SERVER_RESTART_DELAY);
...
/**
 * Ensure powerac.key exists with a secure random key
...
function ensureKeyFileExists(forceNew = false) {
    ...
    try {
        if (forceNew) {
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new powerac.key with secure random key: ${keyPath}`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0);
            }, SERVER_RESTART_DELAY);
        ...
        if (fs.existsSync(keyPath)) {
            ...
            if (!isDefaultKey) {
                Logger.info(`powerac.key exists with custom key: ${keyPath}`);
                return true;
            }
            
            Logger.critical(`Default/invalid key detected in powerac.key. Generating new secure key...`);
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Updated powerac.key with new secure random key: ${keyPath}`);
            ...
            const newKey = generateSecureKey();
            fs.writeFileSync(keyPath, newKey, 'utf8');
            Logger.success(`Created new powerac.key file with secure random key: ${keyPath}`);
            Logger.restart(`The server will now restart to apply the new key.`);
            setTimeout(() => {
                process.exit(0);
            }, SERVER_RESTART_DELAY);
...
/**
 * Ensures PowerAC starts before other resources in server.cfg
 * @return {boolean} Success status
 */
function ensurePowerACStartsFirst() {
    ...
    if (!serverCfgPath) {
        Logger.error('Could not find server.cfg. Please manually ensure PowerAC starts first.');
        return false;
    }
...
/**
 * Install PowerAC protection on all resources
 * @return {void}
 */
function installPowerAC() {
    Logger.info(`Starting ${CURRENT_RESOURCE_NAME} installation...`);
    
    if (!ensureKeyFileExists()) {
        Logger.critical(`Failed to ensure powerac.key exists. Installation may be incomplete.`);
    ...
    for (const resource of resources) {
        ...
        if (hasNewPowerAC(content) && hasKeyFile(content)) {
            // Logger.debug(`${resource.name} already has ${CURRENT_RESOURCE_NAME} protection.`);
            successCount++;
            continue;
        }
        
        const updatedContent = addPowerAC(content);
    ...
    ensurePowerACStartsFirst();
    
    Logger.info(`${CURRENT_RESOURCE_NAME} installation complete. Success: ${successCount}, Failed: ${failCount}`);
}

/**
 * Uninstall PowerAC protection from all resources
 * @return {void}
 */
function uninstallPowerAC() {
    ...
    for (const resource of resources) {
        ...
        if (!hasOldPowerAC(content) && !hasNewPowerAC(content) && !hasKeyFile(content)) {
            // Logger.debug(`${resource.name} doesn't have ${CURRENT_RESOURCE_NAME} protection.`);
            successCount++;
            continue;
        }
        
        const updatedContent = removePowerAC(content);
...
/**
 * Update PowerAC protection on all resources
 * @return {void}
 */
function updatePowerAC() {
...
/**
 * Get the encryption key from powerac.key
 * @return {string} The encryption key to use
 */
function getEncryptionKey() {
    try {
        const keyFile = LoadResourceFile("PowerAC", "powerac.key");
        
        if (!keyFile || keyFile === "") {
            Logger.warning("PowerAC key not found, using temporary key");
        ...
        if (keyFile === "Please Change this if u wont the ac just wont start until u do this delete this message and instead write a random combination of letters like a random password without any spaces" || 
            ...
            Logger.critical("Default PowerAC key detected. Running ensureKeyFileExists() to create a new one.");
            checkAndFixDefaultKey();
            
            const updatedKey = LoadResourceFile("PowerAC", "powerac.key");
        ...
        Logger.error(`Failed to load PowerAC encryption key: ${error.message}`);
...
RegisterCommand('securenewkey', (source, args, raw) => {
    ...
    Logger.info('Forcing regeneration of powerac.key...');
    const success = ensureKeyFileExists(true);
    if (success) {
        Logger.success('Generated new PowerAC encryption key. The server will restart to apply changes.');
    } else {
        ...
        setTimeout(() => {
            ...
            try {
                ...
                try {
                    ...
                    const match = cfg.match(/PowerAC\s*\.\s*Module\s*=\s*\{[\s\S]*?ModuleEnabled\s*=\s*(true|false)/i);
                    if (match && match[1]) {
                ...
                if (!fs.existsSync(path.join(RESOURCES_PATH, 'powerac.key'))) {
                    const keySuccess = ensureKeyFileExists();
                    if (!keySuccess) {
                        Logger.critical(`Could not create or verify powerac.key`);
                ...
                if (moduleEnabled) {
                    Logger.info(`Installing PowerAC protections...`);
                    installPowerAC();
                } else {
                    Logger.info(`Uninstalling PowerAC protections...`);
                    uninstallPowerAC();
                }
            } catch (e) {
