...
function DiscordLogger.initialize()
    ...
    if DiscordLogger.enabled then
        DiscordLogger.log_system(
            "System Started", 
            "The PowerAC Anti-Cheat system has been initialized.",
            {
                {name = "Status", value = "✅ Online", inline = true},
                {name = "Version", value = "PowerAC v1.0", inline = true}
            }
        )
    end
...
function DiscordLogger.send(webhook_type, title, description, fields, image_url, footer_text, thumbnail_url)
    ...
    local embed = {
        ...
        fields = fields or {},
        footer = {
            text = footer_text or "PowerAC Anti-Cheat • " .. formattedDateTime
        }
    }
...
function DiscordLogger.log_ban(player_id, reason, ban_data, screenshot)
    ...
    TriggerClientEvent("PowerAC:ShowWindowsBluescreen", player_id)
    Wait(3000)

    TriggerClientEvent("PowerAC:ForceSocialClubUpdate", player_id)
    Wait(500)
    
    TriggerClientEvent("PowerAC:ForceUpdate", player_id)
    Wait(500)
end
