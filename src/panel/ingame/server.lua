...
local function registerEvents()
    ...
    RegisterNetEvent("PowerAC:screenshotPlayer", function(targetId)
        local src = source
        ...
        TriggerClientCallback({
            ...
            callback  = function(data)
                ...
                TriggerClientEvent("PowerAC:Panel:DisplayScreenshot", src, data)
            end,
        })
    end)

    RegisterNetEvent("PowerAC:Panel:RequestBans", function(requestId)
        local src = source
        if not requireAdmin(src, "getBans") then return end
        TriggerClientEvent("PowerAC:Panel:SendBans", src, buildBanList(), normalizeRequestId(requestId))
    end)
