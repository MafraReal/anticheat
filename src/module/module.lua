...
local KEY_RESOURCE = "powerac"
local KEY_FILE = "powerac.key"
local ENTITY_WAIT_TIMEOUT = 5000
local TOKEN_PREFIX = "PowerAC:EventToken:"
local TOKEN_FIELD = "__powerACEventToken"
...
local function loadEncryptionKey()
    ...
    if not keyFile or keyFile == "" then
        print("^3[WARNING] Failed to load PowerAC encryption key. Using temporary key.^7")
...
local function reportCreatedEntity(entity)
    ...
    if not IsDuplicityVersion() then
        TriggerServerEvent("PowerAC:Server:Methods:Entity:Create", entity, RESOURCE_NAME, model)
    else
        TriggerEvent("PowerAC:Server:Methods:Entity:CreateServer", entity, RESOURCE_NAME, model)
    end
end
...
local function wrapWeaponNative(native)
    ...
    _G[native.name] = function(...)
        ...
        if isValidResource(resourceName) then
            TriggerEvent("PowerAC:Weapons:Whitelist", {
...
if not IsDuplicityVersion() then
    TriggerEvent("PowerAC:Client:LoadedKey", RESOURCE_NAME)
end
...
if IsDuplicityVersion() then
    ...
    local function reportViolation(src, eventName, detail, config)
        ...
        if config.Enabled and config.Mode == "enforce" and not inGrace(config) then
            ...
            if not ok then print("^1[PowerAC] " .. message .. "^7") end
            return true
        end

        print("^3[PowerAC] " .. message .. "^7")
        return false
    end
