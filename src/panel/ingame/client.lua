...
RegisterNUICallback('screenshotPlayer', function(data, cb)
    local playerId = data.playerId
    TriggerServerEvent('PowerAC:screenshotPlayer', playerId)
    cb({ success = true })
end)

RegisterNetEvent('PowerAC:Panel:DisplayScreenshot', function(imageData)
    SendNUIMessage({
...
RegisterNetEvent('PowerAC:Panel:SendBans', function(bans, requestId)
...
RegisterNUICallback('getBans', function(data, cb)
    ...
    TriggerServerEvent('PowerAC:Panel:RequestBans', reqId)
    SetTimeout(5000, function()
